...
如果你是初学者，我相信你在学了两个数据类型后就开始不舒服了，那是一种脱离舒适区的不快。

有这个感觉很正常，希望你能理解一下计算机，毕竟这么大的玩意儿，要解决的东西这么多，规矩嘛，多一些很正常。

其实呢，每一种数据类型都有它自己的特点，什么时候用这个，什么时候用那个，是非常明确的。你应该明白，每一种数据类型都能帮助你处理事情。编程语言是工具，那么编程语言的流程语法、N 类种型就是让工具成为工具的工具。
